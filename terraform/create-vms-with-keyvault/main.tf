...
module "key_vault_secret" {
  ...
  key_vault_id = module.key_vault.key_vault["KV1"].id

  key_vault_secret = merge(
    local.has_linux ? {
      "SSH-PRIVATE-KEY" = one(module.ssh_public_key[*].private_key_pem)
    } : {},
    local.has_windows ? {
      "WINDOWS-ADMIN-PASSWORD" = one(random_password.admin_password[*].result)
    } : {},
  )
}
